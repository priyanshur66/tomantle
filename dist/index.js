...
app.post('/execute-contract', validateContractInput, async (req, res) => {
    try {
        const { contractAddress, contractABI, functionName, functionParams, value = "0" } = req.body;
        console.log(`📝 Executing contract interaction:
            ...
            - Value: ${value} ETH
        `);
        const result = await signAndExecuteContractTx(contractAddress, contractABI, functionName, functionParams, value);
        if (!result) {
...
app.get('/test-contract', async (req, res) => {
    try {
        ...
        const result = await signAndExecuteContractTx(TEST_CONTRACT.address, TEST_CONTRACT.abi, functionName, functionParams, "0");
        if (!result) {
            return res.status(500).json({
                success: false,
